...
@php
    ...
    foreach($prods as $p) {
        $barnTotal['ikat'] += $p->items->sum('ikat');
        $barnTotal['papan'] += $p->items->sum('papan');
        $barnTotal['sisa_butir'] += $p->items->sum('sisa_butir');
    }
...
@else
<div class="card"><div style="padding:48px 24px;text-align:center;color:var(--text-secondary);">Tidak ada produksi pada periode ini.</div></div>
@endif
@push('styles')
<style>
