...
<div class="card">
    ...
    <tr>
        ...
        <td data-label="Ikat" class="price-cell">{{ number_format($st ? $st->ikat : 0, 0, ',', '.') }}</td>
        <td data-label="Papan" class="price-cell">{{ $st ? $st->papan : 0 }}</td>
        <td data-label="Sisa Butir" class="price-cell">{{ $st ? $st->sisa_butir : 0 }}</td>
        <td data-label="Total Butir" class="price-cell" style="font-weight:600;color:var(--gain);">{{ $st ? number_format(($st->ikat * $butirPerPapan * 5) + ($st->papan * $butirPerPapan) + $st->sisa_butir, 0, ',', '.') : '0' }}</td>
    </tr>
    @empty
